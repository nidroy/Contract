    ...
    public partial class YearPlan : Form
    {
        public YearPlan(int id)
        {
            ...
            DataSet ds = DatabaseContext.ExecuteQuery(string.Format(
                "SELECT [number], [price] FROM [Year_plan] JOIN [Year] ON [Year_plan].[year_id] = [Year].[id] WHERE [Year_plan].[id] = {0};", id));
            ...
            ds = DatabaseContext.ExecuteQuery(string.Format(
                "SELECT [task] as [Задача куратора], [contractor_report] as [Отчет подрядчика], [customer_report] as [Отчет куратора] FROM [Year_plan-Month_plan] WHERE [year_plan_id] = {0};", id));
